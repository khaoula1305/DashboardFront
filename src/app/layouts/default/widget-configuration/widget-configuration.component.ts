...
onAddedClick(event){
  this.dashboardWidget.widget.widgetType = this.selectedWidgetType;
  this.dashboardWidget.widget.metaDataSourceDataModels =event[0];
  this.dashboardWidgetService.updateDashboardWidget(this.dashboardWidget.dashboard.id, this.dashboardWidget).subscribe(
    result => this.router.navigate(['/dashboards', this.currentDashboard.id])
  );
  console.log('updated dashboard widget', this.dashboardWidget);
}
